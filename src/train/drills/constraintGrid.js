/**
 * constraintGrid — relational training drill. Wraps the `logicgrid` item family; the
 * staircase level raises the required deduction depth (how many constraints have to be
 * chained before the missing cell is determined) and the constraint count. Kind 'choice'.
 */
 ...
 * index into trial.options. A null/undefined response grades as incorrect.
 * summary(records) expects { trial, correct, rtMs, level }-shaped records.
 */

import * as logicGridFamily from '../../items/logicgrid.js';

export const id = 'constraintGrid';
...
function levelSpec(level, range, explicitLimit, wantOptions) {
  const targetB = clamp(-1.05 + (level - 1) * 0.60, range[0], range[1]);
  return {
    targetB,
    optionCount: wantOptions !== null && wantOptions !== undefined
      ...
      : (level <= 4 ? 6 : 8),
    hints: {
      deductionDepth: clamp(1 + Math.floor(level / 2), 1, 4),
      constraintCount: clamp(2 + level, 3, 9),
      gridSize: clamp(2 + Math.floor(level / 3), 2, 4)
    },
...
export function makeRun(rng, params) {
  if (!rng || typeof rng.fork !== 'function') {
    throw new Error('constraintGrid.makeRun: an rng with fork() is required');
  }
  ...
  const range = bRangeOf(logicGridFamily, FALLBACK_B_RANGE);
  ...
  function build(level, index, spec) {
    if (typeof logicGridFamily.generate !== 'function') {
      throw new Error('constraintGrid: items/logicgrid.js does not export generate()');
    }
    let lastErr = null;
    for (let attempt = 0; attempt < GEN_ATTEMPTS; attempt += 1) {
      ...
      try {
        const item = logicGridFamily.generate(rng.fork(`${id}:${index}:${level}:${attempt}`), {
    ...
    throw new Error(
      `constraintGrid: could not generate a valid logic-grid item at level ${level} ` +
  ...
  return {
    ...
    nextTrial(level) {
      ...
      issuedTrials.set(trialId, {
        ...
        deductionDepth: spec.hints.deductionDepth
      });
      return {
        id: trialId,
        level: lv,
    ...
    grade(trial, response) {
      ...
      return {
        ...
        detail: {
          ...
          deductionDepth: rec ? rec.deductionDepth : null,
