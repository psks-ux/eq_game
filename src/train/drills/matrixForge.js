/**
 * matrixForge — induction training drill. Wraps the `matrix` item family: the staircase
 * level is mapped onto generator difficulty (rule count / abstractness via targetB) and
 * onto option count. Kind 'choice'; the UI calls respond(optionId).
 */
 ...
 * Protocol
 * --------
 * nextTrial(level) -> Trial { stimulus:{svg,width,height}, options:[{id,svg,width,height}] }
 * grade(trial, response) accepts an option id string, an option object ({id}), or an
 * index into trial.options. A null/undefined response grades as incorrect (a miss).
 * summary(records) expects records shaped like { trial, correct, rtMs, level }; every
 * field is optional and any missing field degrades gracefully.
 */

import * as matrixFamily from '../../items/matrix.js';

export const id = 'matrixForge';
...
/* --- local utilities (duplicated per drill on purpose: drills own no shared module) --- */
...
/* --- level -> generator spec --- */

function levelSpec(level, range, explicitLimit, wantOptions) {
  const targetB = clamp(-1.10 + (level - 1) * 0.58, range[0], range[1]);
  return {
    targetB,
    optionCount: wantOptions !== null && wantOptions !== undefined
      ...
      : (level <= 4 ? 6 : 8),
    hints: {
      ruleCount: clamp(1 + Math.floor((level - 1) / 2), 1, 4),
      abstractness: round2(clamp(1 + (level - 1) * 0.42, 1, 4)),
      wmLoad: clamp(2 + Math.floor((level - 1) / 2), 1, 6)
    },
...
export function makeRun(rng, params) {
  if (!rng || typeof rng.fork !== 'function') {
    throw new Error('matrixForge.makeRun: an rng with fork() is required');
  }
  ...
  const range = bRangeOf(matrixFamily, FALLBACK_B_RANGE);
  ...
  function build(level, index, spec) {
    if (typeof matrixFamily.generate !== 'function') {
      throw new Error('matrixForge: items/matrix.js does not export generate()');
    }
    let lastErr = null;
    for (let attempt = 0; attempt < GEN_ATTEMPTS; attempt += 1) {
      ...
      try {
        const item = matrixFamily.generate(rng.fork(`${id}:${index}:${level}:${attempt}`), {
    ...
    throw new Error(
      `matrixForge: could not generate a valid matrix item at level ${level} ` +
