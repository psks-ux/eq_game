/**
 * mentalRotation — spatial training drill. Wraps the `rotation` item family under a
 * deadline: the staircase level raises angular disparity, component count and
 * mirror-foil probability while shortening the time limit. Kind 'choice'.
 */
 ...
 * Protocol
 * --------
 * nextTrial(level) -> Trial { stimulus:{svg,width,height}, options:[...], timeLimitMs }
 * grade(trial, response) accepts an option id string, an option object ({id}) or an
 * index into trial.options. A null/undefined response (deadline expiry) is incorrect.
 * summary(records) expects { trial, correct, rtMs, level }-shaped records.
 */

import * as rotationFamily from '../../items/rotation.js';

export const id = 'mentalRotation';
export const factor = 'spatial';
...
const ANGLES = [45, 90, 120, 135, 150, 165, 180, 180];
...
function validItem(item) {
  ...
  if (!item.prompt || typeof item.prompt.svg !== 'string' || item.prompt.svg.length === 0) return false;
...
function levelSpec(level, range, explicitLimit, wantOptions) {
  ...
  return {
    ...
    optionCount: wantOptions !== null && wantOptions !== undefined
      ...
      : (level <= 4 ? 6 : 8),
    hints: {
      angularDisparity: ANGLES[clamp(level - 1, 0, ANGLES.length - 1)],
      componentCount: clamp(2 + Math.floor((level - 1) / 2), 2, 5),
      mirrorProbability: round2(clamp(0.15 + (level - 1) * 0.08, 0.15, 0.65)),
      allowMirrorFoils: true
    },
    timeLimitMs: explicitLimit !== null ? explicitLimit : clamp(9500 - level * 720, 3200, 9500)
  };
}

export function makeRun(rng, params) {
  if (!rng || typeof rng.fork !== 'function') {
    throw new Error('mentalRotation.makeRun: an rng with fork() is required');
  }
  ...
  const range = bRangeOf(rotationFamily, FALLBACK_B_RANGE);
  ...
  function build(level, index, spec) {
    if (typeof rotationFamily.generate !== 'function') {
      throw new Error('mentalRotation: items/rotation.js does not export generate()');
    }
    let lastErr = null;
    for (let attempt = 0; attempt < GEN_ATTEMPTS; attempt += 1) {
      ...
      try {
        const item = rotationFamily.generate(rng.fork(`${id}:${index}:${level}:${attempt}`), {
    ...
    throw new Error(
      `mentalRotation: could not generate a valid rotation item at level ${level} ` +
  ...
  return {
    ...
    nextTrial(level) {
      ...
      issuedTrials.set(trialId, {
        ...
        angularDisparity: spec.hints.angularDisparity
      });
      return {
        id: trialId,
        level: lv,
        stimulus: {
          svg: item.prompt.svg,
          width: num(item.prompt.width) !== null ? item.prompt.width : null,
          height: num(item.prompt.height) !== null ? item.prompt.height : null
        },
    ...
    grade(trial, response) {
      ...
      return {
        ...
        detail: {
          ...
          angularDisparity: rec ? rec.angularDisparity : null,
