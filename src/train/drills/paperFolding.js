/**
 * paperFolding — spatial training drill. Wraps the `folding` item family; the staircase
 * level raises the number of folds, introduces diagonal folds and adds punches.
 * Kind 'choice'.
 */
...
import * as foldingFamily from '../../items/folding.js';

export const id = 'paperFolding';
...
function levelSpec(level, range, explicitLimit, wantOptions) {
  const targetB = clamp(-1.30 + (level - 1) * 0.56, range[0], range[1]);
  return {
    targetB,
    optionCount: wantOptions !== null && wantOptions !== undefined
      ...
      : (level <= 4 ? 6 : 8),
    hints: {
      foldCount: clamp(1 + Math.floor(level / 2), 1, 4),
      diagonalFolds: level >= 4,
      diagonalProbability: round2(clamp((level - 3) * 0.18, 0, 0.6)),
      punchCount: clamp(1 + Math.floor((level - 1) / 3), 1, 3)
    },
...
export function makeRun(rng, params) {
  if (!rng || typeof rng.fork !== 'function') {
    throw new Error('paperFolding.makeRun: an rng with fork() is required');
  }
  ...
  const range = bRangeOf(foldingFamily, FALLBACK_B_RANGE);
  ...
  function build(level, index, spec) {
    if (typeof foldingFamily.generate !== 'function') {
      throw new Error('paperFolding: items/folding.js does not export generate()');
    }
    let lastErr = null;
    for (let attempt = 0; attempt < GEN_ATTEMPTS; attempt += 1) {
      ...
      try {
        const item = foldingFamily.generate(rng.fork(`${id}:${index}:${level}:${attempt}`), {
    ...
    throw new Error(
      `paperFolding: could not generate a valid folding item at level ${level} ` +
  ...
  return {
    ...
    nextTrial(level) {
      ...
      issuedTrials.set(trialId, {
        ...
        foldCount: spec.hints.foldCount
      });
      return {
        id: trialId,
        level: lv,
    ...
    grade(trial, response) {
      ...
      return {
        ...
        detail: {
          ...
          foldCount: rec ? rec.foldCount : null,
