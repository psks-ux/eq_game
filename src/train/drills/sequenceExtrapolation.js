/**
 * sequenceExtrapolation — induction training drill. Wraps the `series` item family; the
 * staircase level raises the period length of the underlying cycle and, from level 4,
 * interleaves independent streams along the same track. Kind 'choice'.
 */
...
import * as seriesFamily from '../../items/series.js';

export const id = 'sequenceExtrapolation';
...
function levelSpec(level, range, explicitLimit, wantOptions) {
  const targetB = clamp(-1.20 + (level - 1) * 0.57, range[0], range[1]);
  return {
    targetB,
    optionCount: wantOptions !== null && wantOptions !== undefined
      ...
      : (level <= 4 ? 6 : 8),
    hints: {
      period: clamp(2 + Math.floor((level - 1) / 2), 2, 5),
      streams: level >= 4 ? 2 : 1,
      interleaving: clamp(1 + Math.floor((level - 3) / 2), 1, 3),
      termCount: clamp(4 + Math.floor(level / 2), 4, 8)
    },
...
export function makeRun(rng, params) {
  if (!rng || typeof rng.fork !== 'function') {
    throw new Error('sequenceExtrapolation.makeRun: an rng with fork() is required');
  }
  ...
  const range = bRangeOf(seriesFamily, FALLBACK_B_RANGE);
  ...
  function build(level, index, spec) {
    if (typeof seriesFamily.generate !== 'function') {
      throw new Error('sequenceExtrapolation: items/series.js does not export generate()');
    }
    let lastErr = null;
    for (let attempt = 0; attempt < GEN_ATTEMPTS; attempt += 1) {
      ...
      try {
        const item = seriesFamily.generate(rng.fork(`${id}:${index}:${level}:${attempt}`), {
    ...
    throw new Error(
      `sequenceExtrapolation: could not generate a valid series item at level ${level} ` +
  ...
  return {
    ...
    nextTrial(level) {
      ...
      issuedTrials.set(trialId, {
        ...
        period: spec.hints.period,
        streams: spec.hints.streams
      });
      return {
        id: trialId,
        level: lv,
